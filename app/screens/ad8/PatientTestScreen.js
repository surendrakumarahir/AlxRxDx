...
class PatientTestScreen extends Component {
  ...
  generateTableData(questionsData) {
    ...
    this.setState({dataSource: questionsData});
  }
...
const dataQustion =  rowData.answers.map((dataNew, index) => {
  var answerImage = null;
  if (dataNew.isSelected === true) {
  ...
  return (
    <TouchableElement underlayColor={'#cdcdcd'} key={index} onPress={() => this.onAnswerSelected(sectionIndex, index)}>
      <View style={[sharedStyles.answerCellContainer, sharedStyles.paddingRight10]}>
        {answerImage}
        <Text style={sharedStyles.answerText}>{dataNew.title}</Text>
      </View>
    </TouchableElement>
  );
})
