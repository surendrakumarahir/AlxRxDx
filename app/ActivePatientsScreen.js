...
class ActivePatientsScreen extends Component {
  ...
  updateData() {
    AsyncStorage.getItem('@appDataStore:sessionToken').then((value) => {
        if (value !== null){
         // console.log(`${AppConstants.hostName}/api/v1/patients?token=${encodeURIComponent(value)}`)
  ...
  // console.log('nextScreenParams', nextScreenParams);
    return (
      <Swipeout 
      right={swipeBtns}
        autoClose={true}
